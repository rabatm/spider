...
fn scrape_url(url : Url, profondeur: u32) -> Url
{
...
fn scrape_page(url_a_visiter: &Url, path_de_sauvegarde: &str) -> Vec<Url> {
    // Le code qui scrape UNE page ira ici.
    // ...
    // Et à la fin, on retourne la liste des nouveaux liens trouvés.
}
            ...
            match base_url.join(src) {
                Ok(full_url) => {
                        ...
                        url_as_string.ends_with(".gif") || url_as_string.ends_with(".bmp") 
                    {
                        ...
                        let final_path = format!("{}/{}", &args.path, file_name);
                        
                        println!("Image à télécharger : {} -> {}", url_as_string, final_path);
                        ...
                        println!("-> Image sauvegardée dans : {}", final_path);
                        break;
                    }
                }
                Err(e) => {
